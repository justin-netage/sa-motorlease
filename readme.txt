...
* **Carousel cards load on demand.** Only the first `limit` cards (8) are in the page; the remaining vehicle IDs ride along on the strip and the next batch is fetched (new `sa_vf_cards` AJAX action) and appended as the visitor keeps clicking Next or drags towards the end, so the initial HTML stays the same size however many vehicles are flagged. The Next arrow stays live while more remain; a failed fetch re-queues its batch for the next click. Filter asset version bumped to 2.0.1.
...
* **Force-refresh for vehicles PACE changed without bumping `write_date`.** The update pass gates every field — price, attributes, images, categories — on the feed's `write_date` differing from the stored `_vi_write_date` marker. Anything PACE changes without touching `write_date` (deposit amounts are the known case) is therefore invisible to the importer, and the run reports success having skipped the vehicle; editing or deleting the product's attributes changes nothing, because the code that rewrites them sits behind the same gate. The progress UI now accepts `&force_sku=SKU` (clear the marker for one vehicle) or `&force=1` (all vehicles) so the next pass refreshes them in full. Note a full-fleet force re-syncs images too, so expect it to take a while.
