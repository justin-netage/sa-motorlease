...
* **`[sa_featured_vehicles]` shortcode.** A featured-listings carousel scoped to a category — shows WooCommerce "Featured" products (falling back to the newest vehicles in that category), styled with the same cards. Attributes: `category` (id/slug; defaults to the current archive term), `limit` (default 8), `title` (default "Featured Listings").
