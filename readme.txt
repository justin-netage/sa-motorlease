...
* **`[sa_listings_disclaimer]` and `[sa_breadcrumbs]` shortcodes.** The disclaimer text is editable in **SA Motorlease → Settings → Vehicle Listings** (`listings_disclaimer`) and output by `[sa_listings_disclaimer]`; `[sa_breadcrumbs]` renders WooCommerce breadcrumbs. The location-archive takeover renders the featured slider first, then a title + breadcrumb row, the per-location intro and the disclaimer above the filter.
